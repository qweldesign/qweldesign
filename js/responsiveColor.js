...
export default class ResponsiveColor {
  constructor(elems) {
    // 要素を取得
    this.elems = elems || document.querySelectorAll('.responsiveColor');
    if (!this.elems.length) return;

    // 要素を再取得 (Evil Iconsの仕様対策)
    setTimeout(() => {
      this.elems = document.querySelectorAll('.responsiveColor');
      if (!this.elems.length) return;
    }, 1000);
    ...
    window.addEventListener('resize', () => this.sections = this.getSections());
  ...
  change() {
    ...
    colorYiq = this.detectColorYiq(section);
